...
**2026-05-29 — Assumed cf_clearance was needed for EPO Register POST; wasted iterations on Playwright**
DO NOT: use Playwright or attempt to obtain `cf_clearance` to unlock `POST /application` on EPO Register.
Why: CF's Managed Challenge on EPO Register fires specifically on headless Chromium automation signals (navigator.webdriver, CDP fingerprint, headless-GPU HTTP/2 flags). Plain `requests` with a Firefox/Gecko UA has none of these signals — CF applies no challenge at all, and `JSESSIONID` (set by the GET warm) is sufficient for `POST /application` to return a PDF. cf_clearance is a Chromium-JS-challenge token that cannot be obtained by a non-Chromium client and is not needed here. Playwright (headless Chromium, stealth-patched) always triggered the CF challenge and never resolved it. Firefox UA via requests is the only reliable bypass.
How to apply: Use Firefox UA (`Mozilla/5.0 ... Gecko/20100101 Firefox/119.0`) in `_BROWSER_HEADERS`. On 403, create a fresh `requests.Session` (fresh cookie jar) — CF's token-bucket is keyed partly on cookie state, so reusing a poisoned session doesn't help. POST /application with `documentIdentifiers=<doc_id>&number=EP<app_no>` returns the whole document as one PDF. Measured ~50–60× speedup over page-by-page fetches.
